...
#     # suitt.addTest(TestCase(类里面的方法名))
#     # runner = unittest.TextTestRunner()
#     # runner.run(suitt)


# 3. TestLoader:（用来加载测试用，然后结合测试集使用）
...
#     unittest.TextTestRunner().run(suite)
#     pattern 代表加载的py文件 ，*.py代表匹配所有py文件。


# 4.TestRunner（）是测试集的的运行器，
# 可以扩展为TextTestRunner（）和HTMLRunner（）两种，run（s）使用（s是测试用例集合或py文件）
